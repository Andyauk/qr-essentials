		...
		if pumpObject ~= 0 then
			...
			if #(pos - objectPos) < 3.0 then
				...
				if IsControlJustReleased(0, QRCore.Shared.Keybinds['J']) then
					TriggerEvent('qr-waterpump:client:drinking')
					TriggerEvent('qr-alerts:client:callpolice', objectPos, 'pump attack')
				end
			end
		end
...
RegisterNetEvent('qr-waterpump:client:drinking', function()
    if isBusy then
        return
    else
        isBusy = not isBusy
